...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 31, 2019 at 01:54 AM
...
--
-- Stand-in structure for view `anytimefitness_view`
-- (See below for the actual view)
--
CREATE TABLE `anytimefitness_view` (
`eq_name` varchar(30)
,`eq_type` varchar(30)
...
--
-- Structure for view `anytimefitness_view`
--
DROP TABLE IF EXISTS `anytimefitness_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `anytimefitness_view`  AS  select `equipment`.`eq_name` AS `eq_name`,`equipment`.`eq_type` AS `eq_type`,`manager`.`man_Lname` AS `man_Lname`,`member`.`mem_lname` AS `mem_lname`,`member`.`mem_sex` AS `mem_sex` from ((`equipment` join `manager` on((`equipment`.`manager_id` = `manager`.`id`))) join `member` on((`member`.`id` = `equipment`.`member_id`))) ;
